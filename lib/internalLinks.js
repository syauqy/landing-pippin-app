...
export function buildInternalLinksMap() {
  ...
  posts.forEach((post) => {
    ...
    const defaultAnchors = generateDefaultAnchors(title, slug, isPillar);
 ...
 */
function generateDefaultAnchors(title, slug, isPillar) {
  const anchors = [];
  ...
  // For pillar articles, add cluster-based anchors
  if (isPillar) {
    const clusterWords = slug.replace(/-/g, " ");
    anchors.push(`${clusterWords} articles`);
    anchors.push(`learn about ${clusterWords}`);
    anchors.push(`explore ${clusterWords}`);
  }
...
export function getEligibleLinksForPost(
  ...
  if (existingSlugs.length > 0) {
    eligible = allEligible.filter((link) =>
      existingSlugs.includes(link.slug),
    );
  } else {
  ...
  eligibleLinks.forEach((linkData) => {
    ...
    for (const potentialAnchor of linkData.anchors) {
      ...
      if (exactMatch && !usedAnchors.includes(potentialAnchor)) {
        ...
        matched = modifiedContent.replace(exactMatch[0], `[${exactMatch[0]}](${linkData.url})`);
        modifiedContent = matched;
