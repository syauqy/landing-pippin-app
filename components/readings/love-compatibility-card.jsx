...
export function LoveCompatibilityCard({ reading, type }) {
  return (
      ...
      <div className="flex flex-col gap-2">
        ...
        <div className="flex flex-row gap-2 items-center overflow-x-scroll">
          <div className="shrink-0 flex flex-row gap-1 items-center px-2 border border-slate-200 rounded-2xl text-sm text-slate-600">
            <span>🏠</span>
            {reading?.reading?.deep_dive?.journey?.result}
          </div>
          <div className="shrink-0 flex flex-row gap-1 items-center px-2 border border-slate-200 rounded-2xl text-sm text-slate-600">
            <span>🍀</span>
            {reading?.reading?.deep_dive?.fortune?.result}
          </div>
          <div className="shrink-0 flex flex-row gap-1 items-center px-2 border border-slate-200 rounded-2xl text-sm text-slate-600">
            <span>🤝</span>
            {reading?.reading?.deep_dive?.character?.result}
          </div>
          <div className="shrink-0 flex flex-row gap-1 items-center px-2 border border-slate-200 rounded-2xl text-sm text-slate-600">
            <span>❤️</span>
            {reading?.reading?.deep_dive?.destiny?.result}
          </div>
        </div>
      </div>
